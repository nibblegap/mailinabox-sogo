    ...
    def can_provision_for_domain(domain):
        ...
        for rtype, value in [("A", env["PUBLIC_IP"]), ("AAAA", env.get("PUBLIC_IPV6"))]:
            ...
            try:
                ...
                problems[domain] = "DNS isn't configured properly for this domain: DNS resolution failed (%s: %s)." % (
                rtype, str(e) or repr(e))  # NoAnswer's str is empty
                return False
            except Exception as e:
                problems[domain] = "DNS isn't configured properly for this domain: DNS lookup had an error: %s." % str(
                    e)
                return False
            if len(response) != 1 or str(response[0]) != value:
                ...
                rtype, ", ".join(str(r) for r in response))
                return False
...
def provision_certificates(env, agree_to_tos_url=None, logger=None, show_extended_problems=True, force_domains=None,
                           jsonable=False):
    ...
    for domain_list in certs:
        ...
        try:
        ...
        except (client.InvalidDomainName, client.NeedToTakeAction, client.ChallengeFailed, client.RateLimited,
                acme.messages.Error, requests.exceptions.RequestException) as e:
            ret_item.update({
                "result": "error",
            ...
            install_status = install_cert(domain_list[0], cert['cert'].decode("ascii"),
...
def provision_certificates_cmdline():
    ...
    while True:
        ...
        status = provision_certificates(env, agree_to_tos_url=agree_to_tos_url, logger=my_logger,
                                        force_domains=force_domains, show_extended_problems=show_extended_problems)
        ...
        if wait_until:
            ...
            while wait_until > datetime.datetime.now():
                if not headless or first:
                    print ("We have to wait", int(round((wait_until - datetime.datetime.now()).total_seconds())),
...
def check_certificate(domain, ssl_certificate, ssl_private_key, warn_if_expiring_soon=True, rounded_time=False,
    ...
    if not (cert.not_valid_before <= now <= cert.not_valid_after):
        return ("The certificate has expired or is not yet valid. It is valid from %s to %s." % (
    ...
    if "self signed" in verifyoutput:
        ...
        if "unable to get local issuer certificate" in verifyoutput:
            return (
            "The certificate is missing an intermediate chain or the intermediate chain is incorrect or incomplete. (%s)" % verifyoutput,
            None)
