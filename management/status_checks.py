...
def check_service(i, service, env):
    ...
    if service["public"]:
        ...
        # IPv4 failed. Try the private IP to see if the service is running but not accessible (except DNS because a different service runs on the private IP).
            ...
            output.print_error("%s is running but is not publicly accessible at %s:%d." % (
            service['name'], env['PUBLIC_IP'], service['port']))
        else:
...
def check_primary_hostname_dns(domain, env, output, dns_domains, dns_zonefiles):
    ...
    if ns_ips == env['PUBLIC_IP'] + '/' + env['PUBLIC_IP']:
        output.print_ok("Nameserver glue records are correct at registrar. [ns1/ns2.%s ↦ %s]" % (
    ...
    existing_rdns_v6 = query_dns(dns.reversename.from_address(env['PUBLIC_IPV6']), "PTR") if env.get(
...
def check_alias_exists(alias_name, alias, env, output):
    ...
    if alias in mail_aliases:
        if mail_aliases[alias]:
            ...
            output.print_error(
                """You must set the destination of the mail alias for %s to direct email to you or another administrator.""" % alias)
    else:
        output.print_error(
            """You must add a mail alias for %s which directs email to you or another administrator.""" % alias)
...
def check_dns_zone_suggestions(domain, env, output, dns_zonefiles, domains_with_a_records):
    ...
    if "www." + domain in domains_with_a_records:
        output.print_warning(
            """A redirect from 'www.%s' has been disabled for this domain because you have set a custom DNS record on the www subdomain.""" % domain)
...
def what_version_is_this(env):
    ...
    tag = shell("check_output", ["/usr/bin/git", "describe", "--abbrev=0"],
                env={"GIT_DIR": os.path.join(miab_dir, '.git')}).strip()
    return tag
