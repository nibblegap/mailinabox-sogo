...
def open_database(env, with_connection=False):
    conn = MySQLdb.connect(host="localhost",
                           read_default_file='/etc/mysql/debian.cnf',
                           db="mailinabox")
...
def get_mail_users_ex(env, with_archived=False, with_slow_info=False):
    ...
    for email, name, privileges in c.fetchall():
        ...
        if with_slow_info:
            user["mailbox_size"] = utils.du(
                os.path.join(env['STORAGE_ROOT'], 'mail/mailboxes', *reversed(email.split("@"))))
...
def get_mail_aliases(env):
    ...
    c = open_database(env)
    c.execute("""SELECT source, destination, permitted_senders FROM miab_aliases""")
...
def add_mail_user(email, pw, privs, name, env):
    # validate email
    if email.strip() == "":
    ...
    try:
        c.execute("""INSERT INTO miab_users (email, password, name, maildir, privileges) VALUES (%s, %s, %s, %s, %s)""", (email, pw, name, maildir, "\n".join(privs),))
    except:
    ...
    if validate_email(r1, mode='alias') and not is_dcv_source:
        ...
        for line in forwards_to.split("\n"):
            for email in line.split(","):
                ...
                if is_dcv_source and not is_dcv_address(email) and "admin" not in get_mail_user_privileges(email, env,
                                                                                                           empty_on_error=True):
...
def get_required_aliases(env):
    ...
    # Create postmaster@, admin@, abuse@ and sogo@ for all domains we serve
