...
def backup_status(env):
    ...
    if incremental_count > 0 and first_full_size is not None:
        ...
        est_time_of_next_full = min(est_time_of_next_full,
                                    first_full_date + datetime.timedelta(days=config["min_age_in_days"] * 10 + 1))
    ...
    for bak in backups:
        ...
        if bak["full"]:
            ...
            deleted_in = reldate(now, dateutil.parser.parse(bak["date"]) + datetime.timedelta(
                days=config["min_age_in_days"]), "on next daily backup")
            bak["deleted_in"] = deleted_in
    ...
    for bak in backup_status(env)["backups"]:
        if not bak["full"]:
            ...
            if dateutil.parser.parse(bak["date"]) + datetime.timedelta(
                    days=config["min_age_in_days"] * 10 + 1) < datetime.datetime.now(dateutil.tz.tzlocal()):
                return True
            return False
    else:
