...
def build_zone(domain, all_domains, additional_records, www_redirect_domains, env, is_zone=True):
    ...
    if domain == env["PRIMARY_HOSTNAME"]:
        ...
        if env.get("PUBLIC_IPV6"): records.append(
            (None, "AAAA", env["PUBLIC_IPV6"], "Required. Sets the IPv6 address of the box."))
        ...
        for value in build_sshfp_records():
            records.append((None, "SSHFP", value,
    ...
    defaults = [
        (None, "A", env["PUBLIC_IP"],
         "Required. May have a different value. Sets the IP address that %s resolves to for web hosting and other services besides mail. The A record must be present but its value does not affect mail delivery." % domain),
        (None, "AAAA", env.get('PUBLIC_IPV6'),
         "Optional. Sets the IPv6 address that %s resolves to, e.g. for web hosting. (It is not necessary for receiving mail on this domain.)" % domain),
    ]
    if "www." + domain in www_redirect_domains:
        defaults += [
            ("www", "A", env["PUBLIC_IP"],
             "Optional. Sets the IP address that www.%s resolves to so that the box can provide a redirect to the parent domain." % domain),
            ("www", "AAAA", env.get('PUBLIC_IPV6'),
             "Optional. Sets the IPv6 address that www.%s resolves to so that the box can provide a redirect to the parent domain." % domain),
        ]
    ...
    if not has_rec(None, "MX", prefix="10 "):
        records.append((None, "MX", "10 %s." % env["PRIMARY_HOSTNAME"],
                        "Required. Specifies the hostname (and priority) of the machine that handles @%s mail." % domain))
    ...
    if not has_rec(None, "TXT", prefix="v=spf1 "):
        records.append((None, "TXT", 'v=spf1 mx -all',
                        "Recommended. Specifies that only the box is permitted to send @%s mail." % domain))
    ...
    with open(opendkim_record_file) as orf:
        ...
        if not has_rec(m.group(1), "TXT", prefix="v=DKIM1; "):
            records.append((m.group(1), "TXT", val,
                            "Recommended. Provides a way for recipients to verify that this machine sent @%s mail." % domain))

    # Append a DMARC record.
    # Skip if the user has set a DMARC record already.
    if not has_rec("_dmarc", "TXT", prefix="v=DMARC1; "):
        records.append(("_dmarc", "TXT", 'v=DMARC1; p=quarantine',
                        "Recommended. Specifies that mail that does not originate from the box but claims to be from @%s or which does not have a valid DKIM signature is suspect and should be quarantined by the recipient's mail system." % domain))
    ...
    for qname in all_resolvable_qnames:
        if not has_rec(qname, "TXT", prefix="v=spf1 "):
            records.append((qname, "TXT", 'v=spf1 -all',
        ...
        if not has_rec(dmarc_qname, "TXT", prefix="v=DMARC1; "):
            records.append((dmarc_qname, "TXT", 'v=DMARC1; p=reject',
    ...
    if domain != env["PRIMARY_HOSTNAME"]:
        for dav in ("card", "cal"):
            ...
            if not has_rec(qname, "SRV"):
                records.append((qname, "SRV", "0 0 443 " + env["PRIMARY_HOSTNAME"] + ".",
    ...
    for key in ("KSK", "ZSK"):
        ...
        for ext in (".private", ".key"):
            ...
            with open(oldkeyfn + ext, "r") as fr:
                keydata = fr.read()
            keydata = keydata.replace("_domain_",
                                      domain)  # trick ldns-signkey into letting our generic key be used by this zone
            fn = newkeyfn + ext
    ...
    if value is not None and qname != "_secondary_nameserver":
        if rtype in ("A", "AAAA"):
            if value != "local":  # "local" is a special flag for us
                ...
                if rtype == "A" and not isinstance(v, ipaddress.IPv4Address): raise ValueError(
                    "That's an IPv6 address.")
                if rtype == "AAAA" and not isinstance(v, ipaddress.IPv6Address): raise ValueError(
