...
def mail_users_add():
    try:
        return add_mail_user(request.form.get('email', ''), request.form.get('password', ''),
                             request.form.get('privileges', ''), request.form.get('name', ''), env)
    except ValueError as e:
        return (str(e), 400)
...
def mail_user_privs_remove():
    return add_remove_mail_user_privilege(request.form.get('email', ''), request.form.get('privilege', ''), "remove",
                                          env)
...
def dns_set_secondary_nameserver():
    ...
    try:
        return set_secondary_dns(
            [ns.strip() for ns in re.split(r"[, ]+", request.form.get('hostnames') or "") if ns.strip() != ""], env)
    except ValueError as e:
        return (str(e), 400)
        ...
        if request.method == "GET":
            ...
            if rtype in ("A", "AAAA") and value == "":
                value = request.environ.get(
                    "HTTP_X_FORWARDED_FOR")  # normally REMOTE_ADDR but we're behind nginx as a reverse proxy
...
def log_failed_login(request):
    ...
    # message.
    app.logger.warning(
        "Mail-in-a-Box Management Daemon: Failed login attempt from ip %s - timestamp %s" % (ip, time.time()))
